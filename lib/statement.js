...
exports.stater.prototype.mapInput = function (node, address, input) {
  ...
  if (!other)
    return console.log(node.tag, '[WARN] No node at address', address, '- not adding input', input);
...
exports.stater.prototype.input = function (newVal, input) {
  ...
  if (runner) {
    ...
    console.log(this.tag, '[CRITICAL] THE STATE MACHINE IS BROKEN! Lost at', this.state);
    console.log(this.tag, '[CRITICAL] Committing suicide due to broken state.');
    process.exit(1);
  }
};
